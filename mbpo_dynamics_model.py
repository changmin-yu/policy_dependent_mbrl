...
class EnsembleModel(nn.Module):
    def __init__(self, state_size, action_size, reward_size, ensemble_size, hidden_size=200, learning_rate=1e-3, use_decay=False, predict_done=False, 
                 PE_size=None, grad_clip=100):
        super(EnsembleModel, self).__init__()
        self.hidden_size = hidden_size
        if PE_size is not None:
            self.nn1 = EnsembleFC(state_size+action_size+PE_size, hidden_size, ensemble_size, weight_decay=0.000025)
        else:
...
class EnsembleDynamicsModel():
    def __init__(self, network_size, elite_size, state_size, action_size, reward_size=1, hidden_size=200, use_decay=False, predict_done=False, 
                 learning_rate=1e-4, PE_size=None):
        ...
        self.ensemble_model = EnsembleModel(state_size, action_size, reward_size, network_size, hidden_size, use_decay=use_decay, 
                                            predict_done=predict_done, learning_rate=learning_rate, PE_size=PE_size)
    ...
    def train(self, inputs, labels, batch_size=256, holdout_ratio=0., max_epochs_since_update=5):
        ...
        for epoch in itertools.count():
            ...
            for start_pos in range(0, train_inputs.shape[0], batch_size):
                ...
                losses.append(loss)
            print(f'epoch: {epoch} | training loss: {torch.mean(torch.tensor(losses))}')
            train_losses_list.append(torch.mean(torch.tensor(losses)).item())

            mean = None
            logvar = None # save CUDA memory
            losses = None
            with torch.no_grad():
                holdout_mse_losses = []
                test_idx = np.vstack([np.random.permutation(holdout_inputs.shape[1]) for _ in range(self.network_size)])
                for start_pos in range(0, holdout_inputs.shape[1], batch_size):
                    idx = test_idx[:, start_pos:min(start_pos+batch_size, holdout_inputs.shape[1])]
                    holdout_input = torch.cat([holdout_inputs[i, idx[i], :][None, ...] for i in range(holdout_inputs.shape[0])], dim=0).to(device)
                    holdout_label = torch.cat([holdout_labels[i, idx[i], :][None, ...] for i in range(holdout_labels.shape[0])], dim=0).to(device)
                    holdout_mean, holdout_logvar, holdout_done = self.ensemble_model(holdout_input, ret_log_var=True)
                    _, holdout_mse_loss = self.ensemble_model.loss(holdout_mean, holdout_logvar, holdout_label, inc_var_loss=False, done=holdout_done)
                    holdout_mse_losses.append(holdout_mse_loss)
                holdout_mse_losses = torch.mean(torch.cat([h[:, None] for h in holdout_mse_losses], dim=-1), dim=-1).cpu().detach().numpy()
                holdout_mse_losses_list.append(holdout_mse_losses)
                sorted_loss_idx = np.argsort(holdout_mse_losses)
                self.elite_model_idxes = sorted_loss_idx[:self.elite_size].tolist()
                break_train = self._save_best(epoch, holdout_mse_losses)
                print(f'epoch: {epoch} | holdout mse loss: {np.array(holdout_mse_losses)[self.elite_model_idxes].mean()}')
                if break_train:
                    break
