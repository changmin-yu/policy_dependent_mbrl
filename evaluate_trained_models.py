...
def get_args():
    ...
    parser.add_argument('--actor-index', default=1e5, type=float, 
                        help='trajectory index')
    parser.add_argument('--model-index', default=1e5, type=float, 
                        help='cached dynamics model index')
...
def main(cfg):
    ...
    trained_model_fname = os.path.join(cfg.logdir_prefix, cfg.env+cfg.post_fix, 'trained_models', str(int(cfg.model_index))+'.pt')
    trajectory_fname = os.path.join(cfg.logdir_prefix, cfg.env+cfg.post_fix, 'trajectories', str(int(cfg.trajectory_index))+'.pt')
    ...
    eval_inputs = np.concatenate((eval_obs, eval_action), axis=-1)
    ...
    print(f'Trajectory index: {int(cfg.actor_index)} | Model index: {int(cfg.model_index)} | Average loss: {mean_eval_loss:.2f}')
        

if __name__=='__main__':
    cfg = get_args()
    for actor_ind in np.arange(1e5, 2e6, 3e5):
        for model_ind in np.arange(1e5, 2e6, 3e5):
            cfg.actor_index = actor_ind
            cfg.model_index = model_ind
            main(cfg)
