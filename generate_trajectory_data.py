...
def get_args():
    ...
    parser.add_argument('--replay-buffer-capacity', default=1e3, type=float, 
    ...
    parser.add_argument('--trajectory-maxlen', default=1000, type=int, 
                        help='maximum length of trajectory to log into replay buffer')
...
class actor_collect_data:
    def __init__(self, cfg):
        ...
        self.replay_buffer = ReplayBuffer(self.env.observation_space.shape,
                                          ...
                                          self.device, 
                                          cfg.trajectory_maxlen)
        
        self.save_buffer_dir = os.path.join(cfg.save_policy_trajectory_prefix, cfg.env+'_'+cfg.post_fix, 'trajectories')
    ...
    def generate_data(self, save=True):
        ...
        obs_list, action_list, reward_list, next_obs_list, done_list, done_no_max_list = [], [], [], [], [], []
        trajectory_count = 0
        while self.step < self.cfg.num_collection_steps:
            if done or episode_step >= self.env._max_episode_steps:
                ...
                reward_list = np.stack(reward_list, axis=0).reshape(-1, 1)
                next_obs_list = np.stack(next_obs_list, axis=0)
                done_list = np.stack(done_list, axis=0).reshape(-1, 1)
                done_no_max_list = np.stack(done_no_max_list, axis=0).reshape(-1, 1)
                
                self.replay_buffer.add(obs_list, action_list, reward_list, next_obs_list, done_list, done_no_max_list)
                obs_list, action_list, reward_list, next_obs_list, done_list, done_no_max_list = [], [], [], [], [], []
                trajectory_count += 1
            ...
            done_no_max_list.append(done_no_max)
            ...
            if self.cfg.verbose and self.step % 10000 == 0:
                print(f'step: {self.step} | time span: {time.time() - start_time:.2f} | number of trajectories samples: {trajectory_count}')
                start_time = time.time()
